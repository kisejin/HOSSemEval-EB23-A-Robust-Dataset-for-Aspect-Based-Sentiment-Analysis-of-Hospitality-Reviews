...
if __name__ == "__main__":
    ...
    for index in range(1, args.num_epochs + 1):
        ...
        with open(
            os.path.join(args.output_dir_AS, TXT_file(file_pre)),
            "r",
            encoding="utf-8",
        ) as f_pre:
            print(file_pre)
    ...
    for index in range(1, args.num_epochs + 1):
        file_pre = "test_ep_" + str(index)
        with open(
            os.path.join(args.output_dir_T, TXT_file(file_pre)),
            "r",
            encoding="utf-8",
        ) as f_pre:
            print(file_pre)
    ...
    with open(
        os.path.join(args.output_dir_AS, TXT_file(best_ep_AS)),
        "r",
        encoding="utf-8",
    ) as f_AS, open(
        os.path.join(args.output_dir_T, TXT_file(best_ep_T)),
        "r",
        encoding="utf-8",
    ) as f_T:
        Gold_Num = 0
        ...
        F_NULL = (
            (2 * P_NULL * R_NULL) / float(P_NULL + R_NULL) if P_NULL != 0 else 0
        )
