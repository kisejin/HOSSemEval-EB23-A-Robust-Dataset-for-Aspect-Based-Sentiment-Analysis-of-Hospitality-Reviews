    ...
    for ex_index, example in enumerate(tqdm(examples)):
        if tokenize_method == "word_split":
            ...
            for i, token in enumerate(tokens_a):
                if token.startswith("##"):
                    if ner_labels_org[i - 1 - token_bias_num] in [
                        "O",
                        "T",
                        "I",
                    ]:
                        ner_labels_a.append(
                            ner_labels_org[i - 1 - token_bias_num]
                        )
                    else:
        ...
        if tokens_b:
            ...
            _truncate_seq_pair(
                tokens_a, tokens_b, ner_labels_a, max_seq_length - 3
            )
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help="Whether to use pretrained model and continue with previous epoch.",
    )
    ...
    all_ner_label_ids = torch.tensor(
        [f.ner_label_ids for f in train_features], dtype=torch.long
    )
    all_ner_mask = torch.tensor(
        [f.ner_mask for f in train_features], dtype=torch.long
    )
    ...
    if args.eval_test:
        ...
        all_ner_mask = torch.tensor(
            [f.ner_mask for f in test_features], dtype=torch.long
        )
    ...
    if args.use_crf:
        model = BertForTABSAJoint_CRF(
            args.model_name, len(label_list), len(ner_label_list)
        )
    else:
        model = BertForTABSAJoint(
            args.model_name,
            len(label_list),
            len(ner_label_list),
            args.max_seq_length,
    ...
    # checkpoint = torch.load(checkpoint_path + "weight/Epoch:02 SupCL_DistilRoberta_Main_TAKS_Mul.pth")
    # model.load_state_dict(checkpoint["model_state_dict"])
    ...
    start_epoch = 0
    if args.pretrained:
        out_path = os.path.dirname(os.path.normpath(args.output_dir))
        checkpoint_path = out_path + "/checkpoint/"
        onlyfiles = sorted(
            [
                f
                for f in listdir(checkpoint_path)
                if isfile(join(checkpoint_path, f))
            ]
        )
        file_name = os.path.join(checkpoint_path, onlyfiles[-1])

        model, optimizer, start_epoch, history = load_checkpoint(
            model, optimizer, filename=file_name
        )
    ...
    if flag == False:
        with open(output_log_file, "w") as writer:
            if args.eval_test:
                writer.write(
                    "epoch\tglobal_step\tloss\ttest_loss\ner_test_loss\ttest_accuracy\n"
                )
            else:
    ...
    global_step = 0
    epoch = 0 if start_epoch == 0 else start_epoch
    num_train_epochs = int(args.num_train_epochs) - epoch
    for _ in trange(num_train_epochs, desc="Epoch"):
        ...
        if args.eval_test:
            ...
            with open(
                os.path.join(args.output_dir, "test_ep_" + str(epoch) + ".txt"),
                "w",
            ) as f_test:
                f_test.write(
                ...
                for (
                    ...
                    ner_test_tokens = test_tokens[
                        batch_index
                        * args.eval_batch_size : (batch_index + 1)
                    ...
                    if args.use_crf:
                        ...
                        # softmax
                        ner_logits = torch.argmax(
                            F.log_softmax(ner_predict, dim=2), dim=2
                        )
        ...
        if args.eval_test:
            ...
            history["loss"].append(tr_loss / nb_tr_steps)
            history["test_loss"].append(test_loss)
            history["ner_test_loss"].append(ner_test_loss)
        ...
        with open(output_log_file, "a+") as writer:
            ...
            out_path = os.path.dirname(os.path.normpath(args.output_dir))
            saved_path = out_path + "/checkpoint/"
            ...
            torch.save(
                ...
                saved_path + "Epoch:{:0>2} BertLarge.pth".format(epoch),
            )
