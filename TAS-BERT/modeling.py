...
class BertForTABSAJoint(nn.Module):
    def __init__(self, model_name, num_labels, num_ner_labels, max_seq_length):
        ...
        # num_labels is the type sum of 0 & 1
        self.classifier = nn.Linear(config.hidden_size, num_labels)
        # num_ner_labels is the type sum of ner labels: TO or BIO etc
        self.ner_hidden2tag = nn.Linear(config.hidden_size, num_ner_labels)
        self.num_labels = num_labels
        self.num_ner_labels = num_ner_labels
        self.max_seq_length = max_seq_length

    def forward(
        self, input_ids, token_type_ids, attention_mask, labels, ner_labels
    ):
        outputs = self.bert(
            input_ids,
        ...
        ner_loss = ner_loss_fct(
            ner_logits.view(-1, self.num_ner_labels), ner_labels.view(-1)
        )
...
class BertForTABSAJoint_CRF(nn.Module):
    ...
    def forward(
        ...
        ner_loss_list = self.CRF_model(
            ner_logits,
            ner_labels,
            ner_mask.type(torch.ByteTensor).cuda(),
            reduction="none",
        )
        ner_loss = torch.mean(-ner_loss_list)
        ner_predict = self.CRF_model.decode(
            ner_logits, ner_mask.type(torch.ByteTensor).cuda()
        )
