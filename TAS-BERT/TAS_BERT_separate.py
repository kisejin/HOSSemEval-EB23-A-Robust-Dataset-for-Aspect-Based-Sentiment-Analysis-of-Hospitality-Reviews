...
from modeling_split import (
    BertConfig,
    BertForTABSAJoint_AS,
    BertForTABSAJoint_CRF_AS,
    BertForTABSAJoint_CRF_T,
    BertForTABSAJoint_T,
)
...
def main():
    ...
    if args.use_crf:
        if args.subtask == "AS":
            model = BertForTABSAJoint_CRF_AS(
                bert_config, len(label_list), len(ner_label_list)
            )
        else:
            model = BertForTABSAJoint_CRF_T(
                bert_config, len(label_list), len(ner_label_list)
            )
    else:
        if args.subtask == "AS":
            model = BertForTABSAJoint_AS(
                bert_config,
                len(label_list),
                len(ner_label_list),
                args.max_seq_length,
                    ...
                    if args.subtask == "AS":
                        ...
                        if args.use_crf:
                            ...
                            ner_logits = torch.argmax(
                                F.log_softmax(logits, dim=2), dim=2
                            )
